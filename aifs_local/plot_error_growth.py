...
This is one case, not a skill assessment: a single valid time shows the
shape of error growth, not how good the model is on average.
"""
...
def main():
    ...
    print(f"| lead | {' | '.join(f'{p} ({u})' for p, _, _, u in PANELS)} |")
    for lead in forecasts:
        row = " | ".join(f"{scores[p][lead]:.2f}" for p, _, _, _ in PANELS)
        print(f"| +{lead}h | {row} |")
    ...
    fig.suptitle(f"AIFS Single 2.0 — global RMSE vs lead time, valid {valid:%Y-%m-%d %H} UTC "
                 "(one case)", fontsize=11, color=TEXT, x=0.02, ha="left")
